...
/* for dump function */
inline std::list<std::string> split(std::string str, char del) { std::list<std::string> sList; std::string s = ""; for (const auto& c : str) { if (c == del) { sList.emplace_back(s); s = ""; } else { if (c != ' ' || del == ' ') { s += c; } } }sList.emplace_back(s);	return sList; }
